...
def minmax(A):
	...
	for idx, arm in dominant_arm.items():
		max_arr = np.array([])
		for y in directions[idx]:
				y = y.reshape(y.shape[0], 1)
				arm = arm.reshape(arm.shape[0], 1)
				A_temp = (A + np.multiply(arm, np.transpose(arm)))
				val = np.sqrt(np.dot(np.dot(np.transpose(y), np.linalg.inv(A_temp)), y))
				max_arr = np.append(max_arr, val[0][0])
		
		# print(max_arr, max(max_arr))
		min_dict[idx] = max(max_arr)
